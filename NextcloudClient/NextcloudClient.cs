...
namespace NextcloudClient
{
    ...
    public class NextcloudClient : IDisposable
    {
        ...
        public NextcloudClient(string url, string userId, string password)  : this(url, new NetworkCredential(userId, password))
        {
        }
        ...
        /// </summary>
        /// <param name="url">Nextcloud instance URL.</param>
        /// <param name="networkCredential">The password credential.</param>
        public NextcloudClient(string url, NetworkCredential networkCredential)
        {
            if (url == null)
            {
                return;
            ...
            _dav = new WebDavSession(_url, networkCredential, HttpVersion)
            {
...
#region DAV
        ...
        public async Task<bool> Upload(string path, Stream stream)
        {
            return await _dav.UploadFileAsync(GetDavUri(path, false), stream);
        }
        ...
        /// <param name="stream"></param>
        /// <returns><c>true</c>, if upload successful, <c>false</c> otherwise.</returns>
        public async Task<bool> UploadWithWebDavSessionItem(string path, Stream stream)
        {
        ...
        /// <param name="stream"></param>
        /// <returns><c>true</c>, if upload successful, <c>false</c> otherwise.</returns>
        public async Task<bool> Upload(WebDavSessionItem webDavSessionItem, string fileName, Stream stream)
        {
            return await _dav.UploadFileAsync(webDavSessionItem, fileName, stream);
        }
        ...
        /// <param name="progress"></param>
        /// <returns><c>true</c>, if upload successful, <c>false</c> otherwise.</returns>
        public async Task<bool> UploadWithProgressAsync(string path, Stream stream, string contentType, IProgress<WebDavProgress> progress, CancellationToken cancellationToken)
        {
        ...
        public static async Task<bool> CheckUserLogin(string serverUrl, string userId, string password, bool ignoreServerCertificateErrors)
        {
            ...
            var client = new NextcloudClient(serverUrl, new NetworkCredential(){ UserName = userId, Password = password }); //, httpBaseProtocolFilter
...
#region Users
        ...
        public async Task<Uri> GetUserAvatarUrl(string username, int size)
        {
            ...
            try
            {
                if (response != null)
                {
                    var length = response.Content.Headers.ContentLength; // long.Parse(response.Headers["Content-Length"]);
                    if (length > 0)
                    {
                        return url;
                    }
        ...
        private async Task<string> DoApiRequest(string method, string path, Dictionary<string, string> parameters = null)
        {
            ...
            switch (method)
            {
                ...
                default:
                    ...
                    content2.Headers.Add("OCS-APIREQUEST", true.ToString());
